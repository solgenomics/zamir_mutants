...
sub format_il()
{
        my $MAP_URL = "http://www.sgn.cornell.edu/maps/pennellii_il/";
	my $DAVIS_DIR = "http://tgrc.ucdavis.edu/Data/Acc/AccDetail.CFM?ACCESSIONS.AccessionNum=";
	...
	if (isExist("field", @hash_keys) == 1)
	{
            $cells .= &data_element("field", "center", "", "", 1);
            $line  .= &data_element($hash_ref->{"field"});
	}
	if (isExist("scan", @hash_keys) == 1)
	{
            $cells .= &data_element("scan", "center", "", "", 1);
            $line  .= &data_element($hash_ref->{"scan"});
	}
	if (isExist("leaves", @hash_keys) == 1)
	{
            $cells .= &data_element("leaves", "center", "", "", 1);
            $line  .= &data_element($hash_ref->{"leaves"});
	}
	...
	#{	    
	#	$cells = &data_element("field", "center", "", "", 1) . &data_element("scan", "center", "", "", 1) . &data_element("leaves", "center", "", "", 1);
	#	$line  = &data_element($hash_ref->{"field"} . &data_element($hash_ref->{"scan"}) . &data_element($hash_ref->{"leaves"}));
	#}
	#else
	#{
	#	$cells = &data_element("field", "center", "", "", 1) . &data_element("leaves", "center", "", "", 1);
	#	$line  = &data_element($hash_ref->{"field"}) . &data_element($hash_ref->{"leaves"});
	#
	#	
	#}
	...
	{
		if (!($title eq "chromosome_chart") && !($title eq "field") && !($title eq "scan") && !($title eq "leaves"))
		{
	...
	my (@cols, @cols1);
	$cols[0] = &style_stat("North - $north", 100, "#0000AF");
	$cols[1] = &style_stat(" FW - 5' : ", 100, "brown", 0);
	$cols[2] = &bold_stat("Not yet available");
	$cols[3] = &style_stat(" REV - 5' : ", 100, "brown", 0);
	$cols[4] = &bold_stat("Not yet available");
	
	$temp = &li_stat(join("", @cols));
	
	$cols1[0] = &style_stat("South - $south", 100, "#0000AF");
	$cols1[1] = &style_stat(" FW - 5' : ", 100, "brown", 0);
	$cols1[2] = &bold_stat("Not yet available");
	$cols1[3] = &style_stat(" REV - 5' : ", 100, "brown", 0);
	$cols1[4] = &bold_stat("Not yet available");
...
sub form_stat(){
	 ...
	 my $FILENAME  = "qtl.cgi";	
	 my $MAIN_URL  = "http://zamir.sgn.cornell.edu/cgi-bin/IL_cards/$FILENAME";	
...
{
        my $PIC_URL = "http://zamir.sgn.cornell.edu/IL_cards_images/";
...
ENTRY :
	...
	{
		next ENTRY unless ($pic =~ /\.jpg/);
		$pic =~ /.*?_(.*)\.jpg/;		
		
		#$hash{"$1"} = &ac_stat("../../../html/display_pic.html?genotype=$genotype&pic=$pic" , &image_stat("$PIC_URL$genotype/$pic", 120, 120));
                $hash{"$1"} = &ac_stat("../../IL_cards/display_pic.html?genotype=$genotype&pic=$pic" , &image_stat("$PIC_URL$genotype/$pic", 120, 120));
                #############################################################  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	}
...
sub get_folder()
{
        my $PIC_DIR = "/data/local/website/zamir/html/IL_cards_images/";
        ##############################################################  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        #my $PIC_DIR = "C:/Documents and Settings/zamir-bio/My Documents/IL Project/IL_cards/IL/";    
