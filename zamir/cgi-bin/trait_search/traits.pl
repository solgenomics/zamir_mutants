...
sub main()
{
	...
	my $URL = "http://zamir.sgn.cornell.edu/cgi-bin/trait_search/traits.pl";
...
sub display_search()
{
	...
	$text .= "<BR><BR>";
	$text .= $h->button_item("Front page", "location='http://zamir.sgn.cornell.edu/Qtl/Html/home.htm'");
...
sub results()
{
	...
	if (!(@request))
	{
     ...
     $text .= $h->button_item("Back to previous page", "location='traits.pl'");
		 $text .= $h->button_item("Back to front page", "location='http://zamir.sgn.cornell.edu/Qtl/Html/home.htm'");
...
sub format_il()
{
	my $IL_CARD = "http://zamir.sgn.cornell.edu/cgi-bin/IL_cards/qtl.cgi?menu=il_card&il=";
	...
	$t1 .= $h->ak_stat("http://zamir.sgn.cornell.edu/Qtl/Html/experiment_per_trait.htm?page_type=1&tr=undef&ex=Global&ch=$chr&il=$gen_temp", "Global experiment");
	...
	foreach $i (@data)        #foreach experiment
	{
		...
		$tempo .= $h->write($h->ac_stat("http://zamir.sgn.cornell.edu/Qtl/experiment_card.htm?page_type=1&experiment=" . $s->b_translate_ex($experiment), $experiment), 120, "blue") . "<BR><BR>";
		...
		{		
		 	...
		 	if ($count1 < $req_length)
			{
				 ...
				 {
					$first .= $h->ak_stat("http://zamir.sgn.cornell.edu/Qtl/Html/trait_per_experiments.htm?page_type=1&tr=$m_trait&ex=undef&ch=$chr&il=$gen_temp", "$u_trait - all experiments") . "<br>";
				 }
				 else
				 {
		...
		$tempo .= $h->ak_stat("http://zamir.sgn.cornell.edu/Qtl/Html/experiment_per_trait.htm?page_type=1&tr=undef&ex=$m_experiment&ch=$chr&il=$gen_temp", "all traits in this experiment");
