...
$MAIN_URL  = "/cgi-bin/mutation_site/mutant02_search.cgi";
...
sub display_search()
{
	...
	$text  .= h3_stat("Search according to code ID");
	...
	$text  .= h3_stat("Search according to keyword");
	...
	$text .= h3_stat("Search according to categories");	
...
sub get_search()
{
	...
	if ($type == 2)
	{
		for ($i=0; $i<15; $i++)
		{
			...
			if (param("c$j") eq "on")
			{
				...
				$ref = get_codes($high, $mid_num, $ref);		
	...
	{
		$param = param("keyword");
		$ref = get_codes_by_description($param, 1);
	...
	{
		$param = param("code");
		$ref = get_codes_by_description($param, 0);
...
sub results()
{
	...
	for ($i=0; $i<@codes; $i++)
	{
		...
		$temp = get_rows($codes[$i]);		
		
		$text .= format_result($temp);
...
sub get_codes()
{
	...
	if ($mid)
	{
   		$query = "SELECT code FROM mutant02 WHERE $high = $mid";
	}
	else
	{
		$query = "SELECT code FROM mutant02 WHERE $high != 0";
	}
...
sub get_codes_by_description()
{
	...
	if ($type == 1)
	{
   		$query = "SELECT code FROM mutant02 WHERE details like \"%$param%\"";
	}
	else
	{
		$query = "SELECT code FROM mutant02 WHERE code like \"%$param%\"";
	}
...
sub get_pictures()
{
	opendir (THUMB, "/soldb/website/sgn/html/mutation_images/thumbnails"); 
			#or print ("can not open THUMBNAIL directory : $!<br>\n";
	opendir (HALF, "/soldb/website/sgn/html/mutation_images/half_size") ;
			#or print ("can not open HALFSIZE directory : $!<br>\n";	
