...
sub display_search()
{
	...
	$top .= "<IMG SRC=$graphics><A href= $HOME>Home2</A>";
	$top .= center_stat(h1_stat("Search the Tomato Mutant Database")) . "<HR>";
	
	@tgrc = tgrc_genes();
	$ret .="<SELECT NAME="tgrc_gene">";
    foreach $val (@tgrc){
	...
	to_html("Search the database", "$top$ret$form1$form2$text");
...
sub get_rows()
{
	...
	$query = "SELECT * FROM mutant03 WHERE $where";
...
sub get_rows_by_description()
{
	...
	if ($type == 1)
	{
   		$query = "SELECT * FROM mutant03 WHERE details like \"%$param%\"";
	}
	else
	{
		$query = "SELECT * FROM mutant03 WHERE code like \"%$param%\"";
	}
...
sub get_rows_by_code()
{
	...
	my $query;
   	...
   	$query = "SELECT * FROM mutant03 WHERE code = \"$code\"";
...
sub get_tgrc_genes()
{
	...
	my $query;
   		...
   		$query = "SELECT similar_to FROM mutants03;
