...
sub main()
{
	...
	my $URL = "/cgi-bin/mutation_site/IL/Qtl/Ver2/traits.pl";
...
sub display_search()
{
	...
	foreach $i (@traits_arr)
	{
		...
		$t1 = $h->checkbox_item("c_$j", 0) . "  " . $h->bold_stat($s->translate($i)) . " ";
...
sub results()
{
	...
	$text .= $h->center_stat($h->write("Search results", 300, "brown"));
	$text .= "<BR>";	
	
	
	$text .= $h->write("You searched for genotypes which show :<BR><BR>", 175, "blue");
	foreach $i (@request)
	{
		if ($i->[1] == 1)
		{
	...
	$text .= $h->button_item("Back to front page", "location='Qtl/Html/home.htm'");
	$text .= "<BR><BR>";
	$text .= $h->write("$length genotypes found !", 250, "brown");
	$text .= "<BR><HR><BR>";
	my $c = 1;
	my $t="";
	...
	for ($i=0; $i<$act_length; $i++)
	{
	 	...
	 	if ($t)
		{
		 	 $tempo .= $h->r_element($h->data_element(&format_il($h, $s, $struct[$i], \@request, $c) . "<HR>"));
			 $c++;
		}
	}
...
sub format_il()
{
	...
	$t1 .= $h->write($num.")  ". $h->ac_stat("$IL_CARD$gen_temp", $genotype) . "<BR>", 200, "brown") . "<BR>";
	...
	foreach $i (@data)
	{
		...
		$tempo .= $h->write($h->ac_stat("Qtl/experiment_card.htm?page_type=1&experiment=" . $s->b_translate_ex($experiment), $experiment), 150, "blue") . "<BR><BR>";
		...
		for ($j=0; $j<$req_length;$j++)
		{
			...
			if ($value)
			{
				...
				$tempo .= $h->bold_stat($u_trait . " : " . $h->write($value."%",100,$color) . "<BR>");
			}
			else
			{
