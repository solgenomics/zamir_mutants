...
my $FILENAME  = "mutant04_search.pl";

my $DIR       = "/cgi-bin/mutation_site/";
my $PIC_DIR   = "/data/local/website/zamir/html/mutation_images/";
my $MAIN_URL  = "/cgi-bin/mutation_site/$FILENAME";
my $DAVIS_DIR = "http://tgrc.ucdavis.edu/data/acc/GenDetail.aspx?gene=";
my $HOME	=	"http://zamir.sgn.cornell.edu/mutants/";
my $graphics = "http://zamir.sgn.cornell.edu/mutants/graphics/mytomato.gif";

my %high_category = ("seed" => "seed",      
...
elsif ($menu eq "search_by_gene")
{
	results(3);
}
#elsif ($menu eq "display_alleles")
#{
...
sub display_search
{
	...
	$top .= "<table border=0><TR>";
	
	$top .= "<TD rowspan=\"8\" width=200><script type=\"text/javascript\" src=\"/mutants/menu.js\"></script></TD>";
	$top .= "<TD colspan=\"2\">" . center_stat(h1_stat("Search the Tomato Mutant Database")) . "</TD>";
	...
	$text  .= "<br><br>";
	$form1  = data_element(form($text, "search_by_code"));
	
	
	
	$text = "";
	
	$text  .= under_line_stat(h3_stat("Search according to gene name"));
	my @tgrc = get_tgrc_genes();
	my @sorted_tgrc= sort { lc $a cmp lc $b } @tgrc;
	
	$text .="<SELECT NAME=\"gene\">";
	$text .= "<OPTION VALUE=\"\">";
    foreach my $val (@sorted_tgrc){
      $text .= "<OPTION VALUE=\"" . $val . "\">$val" ;
   	}
	$text .="</SELECT>" . "<br>";
	$text  .= submit_item("Search") . reset_item("Clear");
	$text  .= "<br><br>";
	$gene  = data_element(form($text, "search_by_gene"))  ."</TR><TR>";
	...
	$text  .= submit_item("Search") . reset_item("Clear");	
	$text  .= "<br><br>";
	$form2  = "<TD colspan=\"2\">" . (form($text, "search_by_keyword"))  ."</TD></TR><TR>";
	...
	$text  = "<TD colspan=\"2\">" . (form($text,"search_by_category")) ."</TD></TR></TABLE>";

	to_html("Search the database", "$top$form1$gene$form2$text");
...
sub get_tgrc_genes
{
	...
	my $query;
		...
		$query = "SELECT similar_to_tgrc FROM mutant04 where allelic_to_tgrc = \"y\"";
...
sub results
{
	...
	$text .= "<TD colspan=\"2\">" . center_stat(under_line_stat(bold_stat(h1_stat("Search Results")))) ."<BR><BR><BR></TD></TR><TR>";
	
	$text .= "<TD rowspan=\"8\" valign=top width=200><script type=\"text/javascript\" src=\"/mutants/menu.js\"></script></TD><TD>";
	...
	else
	{
		...
		{
			$text .= h3_stat("gene = " .  "\"$req\"");
		}
		else
		{
			$text .= h3_stat("family ID = \"$req\"");		
		}
	}
	...
	$text .= h2_stat("$items mutant accessions found !");
...
sub format_result
{
	...
	my $verified   = $row[17];
	my $gene       = $row[18];
	my $similar_to = $row[19];
	my $allelic_to_tgrc = $row[20];
	my $allelic_to_gmt = $row[21];
	...
	if ($allelic_to_gmt)
	{
	 ...
	 #$body  = submit_item("Known alleles:", $allelic_to_gmt);
 	 #$text .= form($body, "display_alleles"); 
 	 $text .=bold_stat(under_line_stat("Known alleles: <BR>"));   
	  my @alleles= get_alleles($allelic_to_gmt,$code);
	  foreach $a (@alleles)
	  {
	  $text .= bold_stat(" 	   $a <BR>");   
	  }
...
sub get_rows
{
	...
	$query = "SELECT * FROM mutant04 WHERE $where";
...
sub get_rows_by_description
{
	...
	if ($type == 1)
	{
   		$query = "SELECT * FROM mutant04 WHERE details like \"%$param%\"";
	}
	elsif ($type == 3)
	{
   		$query = "SELECT * FROM mutant04 WHERE similar_to_tgrc=\"$param\"";
	}
	
	else
	{
		$query = "SELECT * FROM mutant04 WHERE code like \"%$param%\"";
	}
...
sub get_rows_by_code
{
	...
	my $query;
   	...
   	$query = "SELECT * FROM mutant04 WHERE code = \"$code\"";
...
sub table_element
{
	my ($arg, $width, $space) = @_;
	
	if ($space)
	{
		return "<table width = \"$width\" cellspacing=\"$space\"> $arg </table>";
	}
...
sub bad_format {
	...
	$text .= &bold_stat(&h1_stat("Error"));
	$text .= "<br>";
	$text .= &bold_stat("in order to search by family ID you must enter one of the two letters ('e' or 'n') and then 4 digits !");
	...
	$text .= &bold_stat(&h1_stat("Error"));
	$text .= "<br>";
	$text .= &bold_stat("You must select at least one phenotypic category !");
	$text .= &bold_stat("<BR>Click here to go back");
	$text .= &submit_item("Back");
	$text .= "<BR>\n";
	$text  = &form($text, "get_search");
